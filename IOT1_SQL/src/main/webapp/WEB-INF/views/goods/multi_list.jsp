...
<c:url var="dUrl" value="/goods/destroy" />
<c:url var="eUrl" value="/goods/excel" />
<c:url var="vrUrl" value="/vendor/list" />
<c:url var="vcUrl" value="/vendor/create" />
<c:url var="vdUrl" value="/vendor/destroy" />
<c:url var="vuUrl" value="/vendor/update" />
...
<body>
	...
	<kendo:grid title="회사리스트" name="vendorGrid" height="320px"
		...
		<kendo:dataSource pageSize="20" batch="true">
			<kendo:dataSource-transport>
				...
				<kendo:dataSource-transport-create url="${vcUrl}" dataType="json"
					type="POST" contentType="application/json" />
				<kendo:dataSource-transport-destroy url="${vdUrl}" dataType="json"
					type="POST" contentType="application/json" />
				<kendo:dataSource-transport-update url="${vuUrl}" dataType="json"
					type="POST" contentType="application/json" />
	...
	<kendo:grid title="상품리스트" name="goodsGrid" pageable="true"
		...
		<kendo:dataSource pageSize="20" batch="true">
			<kendo:dataSource-transport>
				<kendo:dataSource-transport-read url="${rUrl}" dataType="json"
					type="POST" contentType="application/json" />
				<kendo:dataSource-transport-create url="${cUrl}" dataType="json"
					type="POST" contentType="application/json" />
				<kendo:dataSource-transport-destroy url="${dUrl}" dataType="json"
					type="POST" contentType="application/json" />
				<kendo:dataSource-transport-update url="${uUrl}" dataType="json"
					type="POST" contentType="application/json" />
				<kendo:dataSource-transport-parameterMap>
					<script>
						function parameterMap(options, type) {
