...
<script>
	...
	function callbackSql(result) {
		...
		if (log == "success") {
			...
			var sqlLog = obj.sqlLog;
			var sqlType = typeof(sqlLog);
			var sql0 = sqlLog[0]
			var sql1 = sql0.substr(sql0.indexOf("from")+5);
			var sql2 = sql1.substr(0,sql1.lastIndexOf("<"));
			if(sqlType=="string"){
			...
			kendoConsole.log(sqlLog + "(CRUD타입: " + type
					+ ")</br>/* Affected rows : " + row + "  찾은 행 : "
					+ gridData.length + "</br>");
		} else {
			kendoConsole.log("/* Affected rows : 0  찾은 행 : 0 </br>"
					+ result.error + "</br>");
		}
	}
	function treeSelect() {
					...
					<kendo:splitter name="horizontal"
						...
						<kendo:splitter-panes>
							...
							<kendo:splitter-pane id="center-pane" collapsible="false">
								<kendo:splitter-pane-content>
									<kendo:splitter layoutChange="changeMiddlePane"
										name="vertical1" orientation="vertical"
										style="height: 100%; width: 100%;">
										<kendo:splitter-panes>
											<kendo:splitter-pane id="top-pane" collapsible="false">
												<div class="pane-content">
